    ...
    <div class="card" style="margin-bottom: 1.5rem;">
        ...
        <form method="GET" action="{{ route('reports.stocks') }}" class="form-grid" style="align-items: flex-end;" id="filter-form">
            ...
            <div style="display: flex; gap: 0.5rem;">
                ...
                <a href="{{ route('reports.stocks.export', request()->all()) }}" class="btn btn-secondary" style="background-color: #10b981; color: white; border-color: #10b981;">
                    Export Excel (CSV)
                </a>
            </div>
        </form>
            ...
            <table class="data-table">
                ...
                <tbody>
                    @forelse($stocks as $stock)
                        <tr>
                            <td><div style="font-weight: 600; color: var(--text-primary);">{{ $stock->branch->name ?? '-' }}</div></td>
                            ...
                            <td style="text-align: right; color: var(--text-secondary);">
                                {{ number_format($stock->quantity * $stock->product->purchase_price, 0, ',', '.') }}
                            </td>
                            <td>
                                @if($stock->quantity == 0)
                    ...
                    @empty
                        <tr>
                            <td colspan="9" style="text-align: center; padding: 2rem;">Belum ada data stok.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
