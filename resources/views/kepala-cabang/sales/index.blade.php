...
@section('content')
    <div class="card">
        <div class="card-header">
            <h3>Riwayat Transaksi Penjualan (Cabang Ini)</h3>
            <div>
                <form id="filterForm" method="GET" action="{{ route('kepala-cabang.transactions.index') }}" style="display: flex; gap: 0.5rem; width: 100%;">
                    ...
                    @if(request('search') != '')
                        <a href="{{ route('kepala-cabang.transactions.index') }}" class="btn btn-secondary btn-sm" style="background: transparent; color: var(--danger); border-color: var(--danger);">Reset</a>
                    @endif
                </form>
            </div>
        </div>
        ...
        <div style="overflow-x: auto;">
            <table class="data-table">
                ...
                <tbody>
                    @forelse($sales as $sale)
                        <tr>
                            ...
                            <td style="text-align: center;">
                                <a href="{{ route('kepala-cabang.transactions.show', $sale->id) }}" class="btn btn-sm btn-secondary" title="Lihat Detail Transaksi">
                    ...
                    @empty
                        <tr>
                            <td colspan="5" style="text-align: center; padding: 2rem;">Belum ada transaksi.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
