    ...
    <div class="card">
        <div class="card-header" style="flex-wrap: wrap; gap: 1rem;">
            <div style="display: flex; align-items: center; gap: 1rem;">
                ...
                <a href="{{ route('kepala-cabang.stock-ins.create') }}" class="btn btn-primary btn-sm">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M12 4.5v15m7.5-7.5h-15" /></svg>
                    Tambah Stok Masuk
                </a>
            </div>
            <div style="display: flex; gap: 0.5rem; flex-wrap: wrap; align-items: center; margin-left: auto;">
                <form id="filterForm" method="GET" action="{{ route('kepala-cabang.stock-ins.index') }}" style="display: flex; gap: 0.5rem;">
                    ...
                    @if(request('search') != '')
                        <a href="{{ route('kepala-cabang.stock-ins.index') }}" class="btn btn-secondary btn-sm" style="background: transparent; color: var(--danger); border-color: var(--danger);">Reset</a>
                    @endif
                </form>
            </div>
        </div>
            ...
            <table class="data-table">
                ...
                <tbody>
                    @forelse($stockIns as $in)
                        <tr>
                            <td>
                                <div>{{ \Carbon\Carbon::parse($in->date)->format('d M Y') }}</div>
                                <div style="font-size: 0.75rem; color: var(--text-muted);">{{ \Carbon\Carbon::parse($in->date)->format('H:i') }} WIB</div>
                            </td>
